...
function centerPathInViewBox(instructions: PathArray, viewBoxWidth: number, viewBoxHeight: number): any[] {
  const xCoordinates: number[] = [];
  const yCoordinates: number[] = [];
  for (let i = 0; i < instructions.length; i++) {
    if (instructions[i] === 'M' || instructions[i] === 'L') {
      xCoordinates.push(instructions[i + 1]);
      yCoordinates.push(instructions[i + 2]);
    }
  }
  ...
  const offset = { x: viewBoxCenter.x - pathCenter.x, y: viewBoxCenter.y - pathCenter.y };

  for (let i = 0; i < instructions.length; i++) {
    if (instructions[i] === 'M' || instructions[i] === 'L') {
      instructions[i + 1] += offset.x;
      instructions[i + 2] += offset.y;
    }
  }
  return instructions;
}
...
const interpolated = new SVGPathCommander(examplePath).interpolate({ x, y, x2, y2 });
